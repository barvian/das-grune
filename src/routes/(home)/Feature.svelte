<script lang="ts">
    ...
    $: if (contentRect && !coords) coords = spring(
		initial,
		{
			stiffness: 0.025,
			damping: 0.35
    ...
    let hovering = false, contentRect: DOMRectReadOnly, loaded = false
...
<a href="#work" class="relative block group" bind:contentRect on:mouseenter={() => hovering = true} on:mouseleave={() => hovering = false} on:mousemove={(e) => {
    ...
    {#if coords}
        <div class="w-48 aspect-square absolute top-0 left-0 overflow-hidden" style:transform="translate(calc({$coords.x}px - 50%), calc({$coords.y}px - 50%))">
